...
- **Subagent Native Delegation (`spawn_subagent`):** Official worker agent tool using the fire-and-wait model (Option A). Enables orchestrator agents to run focused, self-contained subtasks in fresh-context sessions, persisting full message logs and `metadata.json` mapping relations under `sessions/{parentId}/subagents/sub_{toolCallId}/`. Automatically propagates `AbortSignal` for instant subagent cancellation and expects a structured YAML/YAML-like result envelope (status, executive_summary, artifacts, risks). En el frontend, se desactivó la navegación automática forzada al iniciar la delegación y se habilitó un botón de retroceso ("Volver a la Sesión Padre") para retornar ágilmente al chat de origen.
