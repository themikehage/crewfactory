...
- **Subagent Live Console, Model Inheritance & Context Enrichment:** Nested subagent sessions (`spawn_subagent` and `delegate_task`) are unified under `sessionManager.getOrCreateSession()`. This registers subagents dynamically in the global `SessionManager.sessions` registry, allowing the frontend Live Console to receive real-time execution events, streaming updates (`ThinkingBlock` and `ToolCallRow`), and resolve historical logs consistently. Subagents spawned via `spawn_subagent` correctly inherit the parent's `AGENTS.md` rules and available skills (via `loadSkills: true` and parent system prompt context forwarding). Delegated tasks triggered via `delegate_task` (for `agent`, `project`, and `session` targets) are automatically wrapped with a delegation contract forcing them to return structured `status/executive_summary/artifacts/risks` results, ensuring reliable execution parsing and preventing data loss.
...
- **Resolución Unificada de Proyectos:** Las API de archivos, subprocesos de agentes, y el servidor de previsualización (Preview) utilizan una resolución unificada (`resolveProjectDir`) que mapea de forma transparente tanto el nombre amigable del proyecto como su UUID (ID único) al directorio real en disco. Esto previene la duplicación de directorios vacíos creados con el nombre del proyecto.
