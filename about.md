...
- **Tipos Exclusivos de Canal (Debate Stateless vs. Leader-Specialist)**: Los canales funcionan bajo uno de dos mecanismos mutuamente excluyentes configurables desde la UI:
  - **Debate (Sin Estado / Stateless):** El orquestador ejecuta rondas asíncronas en paralelo (`Promise.all`) con llamadas API frescas (`streamSimple`) inyectando el problema original y la transcripción acumulada como estado externo (JSONL). Evita contaminar el historial persistente individual de cada agente.
  - **Líder + Especialistas (Secuencial):** Lógica jerárquica/horizontal persistente basada en colas (`AgentWorkQueue`) y sesiones continuas de los agentes, ideal para la delegación guiada y coordinación centralizada de entregables.
- **Sequential Orchestrator & Configurable Depth**: Dynamic execution depth control with configurable `maxChainDepth` per channel (default 5, editable up to 20 via UI). Powered by a modular, decoupled channel orchestrator containing a thin coordinator (`ChannelOrchestrator`) and dedicated execution modules (`AgentPromptRunner` con soporte de `runStateless` en paralelo para debates, `ChannelNegotiationHandler`, `ResponseParser`, y `ChannelMessagePublisher`).
...
- **Flexible Reply Modes / Roles**: `user-only` (responds to human), `broadcast` (triggers all channel agents), `targeted` (responds to selected peers), and `mention-only` (responds exclusively when explicitly tagged). Para el modo debate, se simplifica el flujo de participación asumiendo a todos los miembros activos como participantes en paralelo por cada ronda.
...
- **Team Session Resolution**: Opening a Team chat resolves its latest Team-scoped session, or creates one when none exists, then navigates to `/teams/{teamId}/session/{sessionId}`. The shared session resolver and layout tabs preserve the Team route prefix.
...
- `components/layout/AppRouter.tsx` — Router y composición actual de la aplicación; concentra autenticación, contexto activo y renderizado de rutas. El estado del laboratorio está desacoplado en `useLaboratoryController`; continúa el plan de extraer rutas declarativas y el contexto de workspace en `plans/app-router-refactor.md`.
