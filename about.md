...
- The MVP scheduler now runs one bounded sequential recipient pass per user message. Broadcast, parallel dispatch, negotiation, arbitration, and agent-triggered recursive rounds remain outside the active execution path.
- On server startup, interrupted pending or running channel executions are finalized as `stalled` with a `server_restart` reason and their open turns are closed as aborted rather than remaining as zombie work.
- Channel configuration mutations are rejected while an execution is active, and member/topology/policy/context changes now advance configuration versioning exactly once per update.
...
- **Image Pipeline Optimization (Plan)**: Roadmap for serving both user-uploaded and agent-generated images efficiently (see `plans/image-pipeline-optimization.md`). Covers HTTP caching with ETag/304, on-the-fly thumbnails via sharp, native `<img>` auth eliminating blob indirection, WebP compression on upload, Range request support, and centralized `ImageManager` across 8 phases.
...
- **Sincronización de Historial en Canales (Split-Brain Prevention):** Sincronización atómica y bidireccional entre la base de datos de mensajes del canal y el historial privado de cada agente (`syncChannelHistory`). Al entrar el turno de un agente, se sobrescribe su historial local con las entradas reales del canal asignando roles nativos (`assistant` para sí mismo, `user` para otros miembros) y eliminando mensajes huérfanos o interrumpidos por cancelaciones de despachos (`abort`), lo cual erradica por completo la desincronización de respuestas y la duplicación cronológica en prompts anidados.
...
- **Configurable Message Rendering Mode**: Support for `streamingRenderMode` toggle ("live" vs "complete"). When set to "complete", partial character-by-character agent message streams are hidden/buffered, showing the complete message only when fully generated. A fixed typing indicator displays active agent names above the chat input to maintain status awareness.
- **Channel UX Fixes (Plan)**: Addressing message bursting perception (`showThinking`/`showTools` defaults, streaming mode typing feedback) and scroll locking during agent streaming (see `plans/fix-channel-bursting-scroll.md`). 4 phases targeting scroll-position-aware auto-scroll, phase-aware typing indicators, and more informative defaults.
- **Channel Persistence & Recovery (Plan)**: Fixing three critical gaps: unified channel workspace (agents write to `channels/{id}/workspace/` instead of agent directory), persistent tool executions (log rotation awareness, cursor pagination, tool call state recovery), and stuck session recovery (proper session linkage, stall banners, retry UI). See `plans/fix-channel-workspace-persistence-recovery.md`.
...
- Channel history can safely replay prior agent responses that lack provider token usage; context sizing falls back to content estimation instead of interrupting the next channel turn.
...
- Terminal activities are rebuilt from the latest session execution's durable event log after a reload or WebSocket reconnection.
- A durable turn is marked completed only after its final channel message has been projected to the legacy conversation stream.
