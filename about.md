...
- **Team Types (current state)**: Teams persist a `teamType`: `Negotiation` uses the parallel stateless debate/consensus loop, while `Orchestration` currently invokes only its designated leader through a stateless stub. Type changes are still permitted in the current implementation; the planned isolation and tool-capable orchestration work is documented in `plans/team-type-isolation-and-orchestration.md`.
