...
- **Team Session Resolution**: Opening an Orchestration Team chat resolves its latest Team-scoped session or creates one, then navigates to `/teams/{teamId}/session/{sessionId}`. Negotiation teams skip session resolver allocation (resolving `sessionId` as `null`), routing immediately to use direct team message histories over WebSockets.
