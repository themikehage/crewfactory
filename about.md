...
- **Dynamic Team Support & Negotiation Robustness:** The `manage_factory` tool natively understands two types of Teams: Orchestration Teams (managed by a persistent leader agent utilizing `delegate_task` to specialist members) and Negotiation Teams (stateless parallel debate loops with arbitration and consensus protocol rules). Includes `send` action to dispatch team prompts asynchronously and `member` action to manage agent assignments. Refactored the debate loop with a trivial/non-substantive message guard (e.g. greeting filter returning guidance) and structured round 1 debate context injection to prevent loops/re-presentations. Parameterized the system prompt compilation cache to include arbiter and output modes, preventing prompt bleed between roles.
