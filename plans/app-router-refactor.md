...
- [x] 5. Extraer `useLaboratoryController` y los modales de laboratorio a un límite propio. Mover fetches, suscripción `experiment_status`, selección de ejecución, acciones de ejecutar/detener/juzgar/exportar/borrar y sus tipos fuera del router.
- [ ] 6. Repartir el renderizado en route elements pequeños: `ChatRoute`, `DelegationsRoute`, `WorkspaceRoute`, `LaboratoryRoute` y rutas administrativas. Cada uno leerá params/contexto y pasará solo las props necesarias a la página final. El contenido ya está dividido en límites administrativo, laboratorio y contexto/chat; falta que cada uno lea params nativos en vez del estado transitorio.
...
- 2026-07-19: se extrajeron `AppRouteContent` y `LaboratoryModals`, de modo que `AppRouter` solo compone guardia, providers, layout y coordinación. Los consumidores directos de chat, selector de modelo, canal y equipo usan ya `useNavigate`. El chequeo de tipos y build del cliente pasan.
- 2026-07-19: el árbol declarativo ahora usa `AppRouter` como layout route y `Outlet` para sus hojas. `RouteRuntimeProvider` comparte el estado mínimo del shell con esas hojas; se eliminó el último consumidor de `useRouter()`. El chequeo de tipos y build del cliente pasan.
- 2026-07-19: el contrato `Route` y `parseRoute` se trasladaron a `router/route-state.ts`. Ya no existe un hook de router de compatibilidad; el parser queda explícitamente aislado como estado transitorio mientras `MainLayout` y sus subcomponentes migran a params nativos.
- 2026-07-19: `AppRouteContent` se redujo a un despachador y las páginas viven en `AdministrativeRoute`, `LaboratoryRoute` y `ContextRoute`, con guardias estrictos de la unión `Route`. El chequeo de tipos y build del cliente pasan.
