    ...
    actions: {
      ...
      upsert: {
        ...
        params: {
          ...
          members: { type: "array", required: true, description: "Array of channel members aligned with topology assignments" },
          topology: { type: "object", required: true, description: "Versioned topology with kind, schedulerMode, assignments, entry point and terminal owner" },
          context: { type: "array", required: false, description: "Array of { key, value } context variables injected into prompts" },
          maxChainDepth: { type: "number", required: false, description: "Maximum depth of agent chain-of-thought (default 5)" },
          showThinking: { type: "boolean", required: false, description: "Show agent thinking/scratchpad in UI" },
          showTools: { type: "boolean", required: false, description: "Show tool calls in UI" },
          streamingRenderMode: { type: "string", required: false, description: "Render mode: live or complete" },
          delegationPattern: { type: "string", required: false, description: "Delegation pattern for the channel" },
