...
function clientLog(level: string, message: string, data?: any) {
...
export function useLLMChannel(sessionId: string | null = "llm_channel"): UseLLMChannelReturn {
  ...
  useEffect(() => {
    ...
    const unsubError = subscribe("llm_error", (data: unknown) => {
      ...
      clientLog("ERROR", `[LLMChannel] Received error response for ${evt.requestId}, error: ${evt.error}`);
      const pending = pendingRequests.current.get(evt.requestId);
      if (pending) {
        pendingRequests.current.delete(evt.requestId);
        if (mountedRef.current) {
  ...
  const sendRequest = useCallback(async (options: LLMRequestOptions): Promise<string> => {
    if (!connected) {
      clientLog("WARN", "[LLMChannel] Cannot sendRequest because connected is false");
      throw new Error("WebSocket not connected");
    }

    const requestId = `req_${Date.now()}_${Math.random().toString(36).slice(2)}`;
    clientLog("INFO", `[LLMChannel] sendRequest starting. requestId: ${requestId}, model: ${options.model}`);
