...
export function SettingsPage() {
  ...
  const handleExportBackup = async () => {
    ...
    try {
      ...
      setError(err.message || "Error exporting backup");
    } finally {
      setExporting(false);
    }
  };
  ...
  const handleImportBackup = async (skipModal = false) => {
    ...
    try {
      ...
      setImportError(err.message || "Error importing backup");
    } finally {
      setImporting(false);
    }
  };
    ...
    <div className="h-full flex flex-col bg-bg">
        ...
        <div className="max-w-2xl mx-auto p-3 sm:p-6 space-y-6">
          ...
          {activeTab === "general" && (
            <div className="space-y-6">
              ...
              <div className="bg-surface rounded-lg p-4 border border-surface-hover/30 space-y-4">
                <div>
                  <h3 className="text-text-primary font-semibold text-sm">Backup & Portability</h3>
                  <p className="text-text-secondary text-[11px] mt-0.5">
                    Export your configuration and workspaces or import a zip backup.
                  </p>
                </div>

                <div className="border-t border-surface-hover/30 pt-3 space-y-3">
                  <div className="text-xs font-semibold text-text-primary">Export Options</div>
                  ...
                  <button
                    ...
                    disabled={exporting}
                    className="text-xs bg-accent/10 text-accent hover:bg-accent/20 border border-accent/25 px-4 py-2 rounded-lg font-semibold transition-all cursor-pointer disabled:opacity-50"
                  >
                ...
                <div className="border-t border-surface-hover/30 pt-3 space-y-3">
                  <div className="text-xs font-semibold text-text-primary">Import Backup</div>
                  
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-[10px] text-text-secondary uppercase font-bold tracking-wider">Import Mode</label>
                      <select
                        value={importMode}
                        onChange={(e) => setImportMode(e.target.value as "merge" | "overwrite")}
                        className="w-full px-3 py-1.5 bg-bg border border-surface-hover rounded-lg text-text-primary outline-none focus:border-accent transition-colors text-xs"
                      >
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] text-text-secondary uppercase font-bold tracking-wider">Select ZIP File</label>
                      <input
                        type="file"
                        accept=".zip"
                        onChange={(e) => {
                        ...
                        className="w-full text-xs text-text-secondary file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-surface-hover/30 file:text-text-primary hover:file:bg-surface-hover/50 file:cursor-pointer"
                      />
                    </div>
                  </div>
                  ...
                  {importSuccess && (
                    <div className="p-3 bg-success/10 border border-success/20 rounded-lg text-success text-xs">
                      {importSuccess}
                    </div>
                  )}
                    ...
                    <button
                      ...
                      className={`text-xs px-4 py-2 rounded-lg font-semibold transition-all cursor-pointer disabled:opacity-50 ${
                        importMode === "overwrite"
                          ? "bg-error/10 text-error hover:bg-error/20 border border-error/25"
      ...
      {showOverwriteModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-surface border border-surface-hover/80 rounded-xl p-5 max-w-md w-full space-y-4 shadow-xl">
            <h3 className="text-error font-bold text-base flex items-center gap-2">
            ...
            <p className="text-text-secondary text-xs leading-relaxed">
              You are about to restore a backup using <strong className="text-text-primary">Overwrite</strong> mode. This will permanently delete all your existing configuration, credentials, agents, channels, and projects.
            </p>
            
            <div className="bg-accent/5 border border-accent/20 rounded-lg p-3 space-y-2">
              <div className="text-accent font-semibold text-xs">Safe Recommendation:</div>
              ...
              <button
                ...
                className="w-full text-center text-[11px] font-semibold text-accent hover:text-accent/80 border border-accent/25 hover:bg-accent/5 py-1.5 rounded-md transition-all cursor-pointer"
              >
                Download Backup Now
              </button>
            </div>
            ...
            <div className="space-y-1.5">
              <label className="text-text-secondary text-[10px] uppercase font-bold tracking-wider">
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-bg border border-surface-hover rounded-lg text-text-primary outline-none focus:border-error transition-colors text-sm uppercase"
              />
            </div>
            ...
            <div className="flex gap-2 justify-end pt-2">
              <button
                ...
                className="text-xs bg-surface-hover/20 text-text-secondary hover:text-text-primary border border-surface-hover/30 px-3.5 py-2 rounded-lg font-semibold transition-all cursor-pointer"
              >
                Cancel
              </button>
              <button
                ...
                className="text-xs bg-error/10 text-error hover:bg-error/20 border border-error/25 px-3.5 py-2 rounded-lg font-semibold transition-all cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed"
              >
                Confirm & Wipe
              </button>
            </div>
