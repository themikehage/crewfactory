  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 max-w-4xl mx-auto w-full">
      <div className="flex items-center justify-between pb-4 border-b border-input/60">
        ...
        <div className="text-[10px] px-2 py-0.5 rounded-full bg-card border border-input/80 text-muted-foreground">
          {tasks.length} {tasks.length === 1 ? "tarea" : "tareas"}
        </div>
      </div>

      {tasks.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 text-center text-muted-foreground/60 text-xs">
          <svg width="24" height="24" viewBox="0 0 20 20" fill="currentColor" className="mb-2 text-muted-foreground/40">
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="text-muted-foreground/50 font-normal">delegó a</span>
                  <span className="font-semibold text-primary">@{task.assignedToName}</span>
                  <span className="text-[10px] bg-primary/10 border border-primary/20 text-primary px-1.5 py-0.2 rounded font-medium">
