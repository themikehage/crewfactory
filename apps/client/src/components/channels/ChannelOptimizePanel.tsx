  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 max-w-4xl mx-auto w-full">
      <div className="flex items-center justify-between pb-4 border-b border-surface-hover/60">
        <div>
          <h3 className="text-sm font-semibold text-text-primary">Optimización de Prompts (Meta-Loop)</h3>
      ...
      {running && history.length === 0 && (
        <div className="p-12 bg-surface border border-surface-hover rounded-2xl flex flex-col items-center justify-center text-center space-y-3">
          ...
          <div className="space-y-1">
            <p className="text-xs font-semibold text-text-primary">Iniciando Optimización...</p>
      ...
      {history.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-1 space-y-3">
            <h4 className="text-[10px] text-text-secondary font-bold uppercase tracking-wider">Iteraciones</h4>
                ...
                <button
                  ...
                  className={`w-full text-left p-3.5 rounded-xl border transition-all flex items-center justify-between cursor-pointer ${
                    selectedEntry?.iteration === entry.iteration
                      ? "bg-surface border-accent/30 text-text-primary"
                      : "bg-surface/50 border-surface-hover/40 text-text-secondary hover:text-text-primary hover:border-surface-hover"
                  }`}
                >
                  ...
                  <div className="text-xs font-bold text-accent">{entry.avgScore.toFixed(1)}%</div>
                </button>
              ))}
            </div>

            {running && (
              <div className="p-4 bg-accent/5 border border-accent/15 rounded-xl flex items-center gap-2.5 text-[10px] text-accent">
          ...
          <div className="md:col-span-2 space-y-4">
            {selectedEntry && (
              <div className="bg-surface border border-surface-hover rounded-2xl p-5 space-y-4">
                <div className="flex items-center justify-between border-b border-surface-hover/60 pb-3">
                  <div>
                    <h4 className="text-xs font-bold text-text-primary">Detalle de Prompts Sugeridos</h4>
                  ...
                  <div className="text-xs font-extrabold bg-accent/10 border border-accent/20 px-3 py-1 rounded-full text-accent">
                ...
                <div className="space-y-3">
                  {Object.entries(selectedEntry.prompts).map(([agentId, prompt]) => (
                    <div key={agentId} className="space-y-1">
                      ...
                      <pre className="p-4 bg-bg/55 border border-surface-hover/40 rounded-xl text-[10px] text-text-primary font-mono whitespace-pre-wrap leading-relaxed max-h-96 overflow-y-auto select-text">
                        {prompt}
                      </pre>
                    </div>
                  ))}
        ...
        !running && (
          <div className="p-12 bg-surface border border-surface-hover rounded-2xl flex flex-col items-center justify-center text-center max-w-lg mx-auto space-y-4">
            ...
            <div className="space-y-1">
              <p className="text-xs font-semibold text-text-primary">Sin Historial de Optimización</p>
